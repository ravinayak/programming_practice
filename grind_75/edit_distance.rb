...
def edit_distance_optimized(word1:, word2:)
  ...
  # dp[i][j] = dp[i-1][j-1] + 1 OR
  # dp[i][j] = [ dp[i-1][j]], dp[i-1][j-1], dp[i][j-1] ].max + 1
  ...
  (1..m).each do |i|
    ...
    (1..n).each do |j|
      if word1[i - 1] == word2[j - 1] # ith char of word1 = jth char of word2
        ...
        curr_row[j] = [
          prev_row[j] + 1, # [i-1, j] => 1 deletion from [i] => [i - 1]
          # [i-1, j-1] => Substitute ith char of word1 with jth char of word2
          prev_row[j - 1] + 1,
          curr_row[j - 1] + 1 # [i, j-1] => 1 addition at the end result
        ].min
      end
    end
