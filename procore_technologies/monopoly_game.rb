...
class BoardSpace
	...
	def buy_property(player)
		puts "Player :: #{player.name} has purchased the property #{self.name}"
	...
	def auction_property(game)
		...
		if current_winner
			...
			puts "Winner :: #{current_winner.name} has won the auction for property #{self.name}"
		end
	end
...
class Property < BoardSpace
	...
	attr_accessor :name, :price, :owner, :houses, :hotel, :mortgaged, :color_group, :rent_levels, :mortgaged
...
class ChanceCard < Card
	...
	def self.create_deck
		[
			Card.new('Advance to Go Position - 200$') do |player, game|
			...
			Card.new('Give player 1 get out of jail card') do |player, game|
			...
			Card.new('You have been selected for street repairs, 25$ for houses, 100$ for hotel') do |player, game|
				total_cost = 0
				player.properties.each do |property|
					next unless property.is_a?(Property)
			...
			Card.new('Bank has paid you a dividend of 50$') do |player, game|
...
class CustomJestCard < Card
	...
	def self.create_deck
		[
			Card.new('You have inherited 100$') do |player, game|
				player.update_money(100)
			end,
			Card.new('You have advanced to Go Position Collect 200$') do |player, game|
				player.update_money(MonopolyGame::SALARY)
				player.position = MonopolyGame::GO_POSITION
			end,
			Card.new('Bank has made an error in your favor collect 200$') do |player, game|
				player.update_money(MonopolyGame::SALARY)
			end,
			Card.new('From Sale of Stock 50$') do |player, game|
				player.update_money(50)
			end,
			Card.new('You have won 2nd prize in beauty contest 10$') do |player, game|
				player.update_money(10)
			end,
			Card.new('Holiday Fund matures - 100$') do |player, game|
				player.update_money(100)
			end,
			Card.new('Life Insurance matures - 100$') do |player, game|
				player.update_money(100)
			end,
			Card.new('Income Tax Refund 50$') do |player, game|
				player.update_money(50)
			end,
			Card.new('Consultancy Fees 25$') do |player, game|
				player.update_money(25)
			end,
			Card.new('Hospital Fees - 100$') do |player, game|
				player.update_money(-100)
			end,
			Card.new('School Fees - 50$') do |player, game|
				player.update_money(-50)
			end,
			Card.new('Doctor Fees - 50$') do |player, game|
				player.update_money(-50)
			end,
			Card.new('Go to Jail') do |player, game|
				game.go_to_jail(player)
			end,
			Card.new('Player has earned Get out of Jail Card') do |player, game|
				player.add_get_out_of_jail_card
			end,
			Card.new('You have been selected for Street Repairs') do |player, game|
				total_cost = 0
				player.properties.each do |property|
					next unless property.is_a?(Property)
